...
export default function Board() {
  ...
  const [activeButton, setActiveButton] = useState<Record<number, number>>({});
  ...
  // async function onClickDelete(index: number, boardIdx: number) {
  // 	setActiveButton((prev) => ({
  // 		...prev,
  // 		[index]: 2,
  // 	}));
                ...
                <React.Fragment key={content.idx}>
                  <Item>
                    ...
                    {activeButton[index] === 1 ? (
                      <Input value={content.boardContent} />
                    ) : (
                      <span>{content.boardContent}</span>
                    )}
                    <br />({formattedDate})<Button>수정</Button>
                    <Button>삭제</Button>
                  </Item>
                </React.Fragment>
              );
